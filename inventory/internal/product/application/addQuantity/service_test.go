...
func Test_AddQuantityService(t *testing.T) {
	...
	productRepositoryMock.On("Update", mock.Anything, mock.AnythingOfType("domain.Product")).Return(nil).Once().Run(func(args mock.Arguments) {
		...
		assert.Equal(t, 6, product.Quantity.Primitive())
	})
	...
	addQuantityService := NewAddQuantityService(productRepositoryMock, eventBusMock)
	err := addQuantityService.Add(ctx, productID, quantity)
